 ...
 *
 * Note: On other phones the storages other than RMS may be available, 
 * so assume that this implementation is the Siemens-specific.
 *
    ...
    public synchronized void store(Address address) 
        ...
        try {
            ...
            if (enum.hasNextElement()) {
                ...
                recordStore.setRecord(enum.nextRecordId(), serializedAddress, 0, 
                        serializedAddress.length);
            } else {
            
                // Put the new record
                recordStore.addRecord(serializedAddress, 0, serializedAddress.length);
            }
