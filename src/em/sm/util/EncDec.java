...
public class EncDec {
     ...
     */
    public static final void fillBytes(byte content, byte[] bytes, 
     ...
     */
    public static final void encodeIntegerToBytes(int integer, byte[] bytes, 
            int bytesOffset) {
     ...
     */
    public static final void encodeIntegersToBytes(
            int[] integers, int integersOffset, int length, 
            byte[] bytes, int bytesOffset) {
                
        for (int i = 0; i < length; i++) {
            encodeIntegerToBytes (integers[integersOffset + i], bytes, 
                    bytesOffset + (i << 2));
        }
    }
     ...
     */
    public static final int encodeBytesToInteger(byte[] bytes, 
            int bytesOffset) {
                
        return ((bytes[bytesOffset]) << 24) 
                + ((bytes[bytesOffset + 1] & 0xff) << 16)
                + ((bytes[bytesOffset + 2] & 0xff) << 8)
                + (bytes[bytesOffset + 3] & 0xff);
    }
     ...
     */
    public static final void encodeBytesToIntegers(byte[] bytes, 
            int bytesOffset, int[] integers, int integersOffset, int length) {
                
        for (int i = 0; i < length; i++) {
            integers[integersOffset + 1] = 
                    encodeBytesToInteger(bytes, bytesOffset + (i << 2));
        }
    }
     ...
     * @param bytesOffset The byte[] offset.
     */
    public static final void encodeLongToBytes(long longValue, byte[] bytes, 
            int bytesOffset) {
	...
	return ((bytes[bytesOffset] & 0xffL) << 56) 
                | ((bytes[bytesOffset + 1] & 0xffL) << 48) 
                | ((bytes[bytesOffset + 2] & 0xffL) << 40) 
                | ((bytes[bytesOffset + 3] & 0xffL) << 32) 
                | ((bytes[bytesOffset + 4] & 0xffL) << 24) 
                | ((bytes[bytesOffset + 5] & 0xffL) << 16) 
                | ((bytes[bytesOffset + 6] & 0xffL) << 8) 
                | (bytes[bytesOffset + 7] & 0xffL);
    }
    ...
    public static final void decodeHexString(String string, byte[] bytes) {
        ...
        for (int i = 0; i < length; i ++) {
            ...
            bytes[index] = (byte)(bytes[index] 
                    | ((charValue < 0) ? 0 : charValue << (((i + 1) % 2) * 4)));
        }
    }
